...
namespace Instech.Framework.Data
{
    ...
    public class ExcelData : IConfigData
    {
        ...
        public float GetFloat(string field)
        {
            ...
            if (!float.TryParse(_curLine[fieldIdx - 1], out var ret) && !string.IsNullOrWhiteSpace(_curLine[fieldIdx - 1]))
            {
                Logger.LogWarning(LogModule.Data, $"不能转换为浮点数:{_curLine[fieldIdx - 1]}");
            }
            return ret;
        }
